...
- **Score:** one unbounded number the reader can compare with someone else, in `src/lib/stats/score.ts`. One number, no levels: a level needs boundaries and a score that climbs forever has none. It is a sum over everything they know, so learning more always adds to it. It is snapshotted into each attempt record, because item state says what is true today and the past cannot be recomputed from it.
