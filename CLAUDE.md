...
- **Local-first.** All progress lives in the reader's browser (IndexedDB), and that copy is the source of truth. The site is static, hosted on Vercel at yomukana.app. There is no app server.
- **Sync is opt-in.** A reader who signs in (email and password, Neon Auth) gets a copy in Neon Postgres, Frankfurt, read and written from the browser through the Neon Data API. Row-level security keys every row to the token's user, so the browser only ever holds a short-lived token, never a connection string. Auth is reached through this site's own `/api/auth`, forwarded to Neon Auth by the edge middleware in `middleware.ts` in production and by the Vite proxy locally, so the session cookie is first-party. Safari throws away the third-party cookie Neon's own address would set, which signed readers out the moment they signed in. A reader who never signs in never loads the sync code and never contacts Neon. Two Neon branches: `production` backs the live site, `dev` backs `bun run dev` and every test, so testing never touches a real reader. `.env.local` holds `dev`, `.env.prod.local` holds `production`, and Vercel's environment variables name `production` for the live build. Schema and migrations are Drizzle, dev-time only, under `drizzle/`: `bun run db:generate`, then `bun run db:migrate` (dev), and only once that works, `bun run db:migrate:production`. Writes that need a rule checked first go through a Neon Function, `functions/api/`, deployed per branch with `neon deploy --branch <name>` and reached at `/api/v1` the same way auth is. It checks names against the naughty-words list in every language it has, so names and display names are written only there: the Data API may touch a profile's score and nothing else. It connects as the database owner, so every query in it scopes itself to the caller's verified token. Merge rules live in `src/lib/sync/merge.ts` and are mirrored by triggers in `drizzle/migrations/0001_merge_rules.sql`: newest review wins per item, attempts are append-only.
- **Friends board, for signed-in readers.** Every signed-in reader gets a profile with a random username (changeable in settings) and their score, sent with each sync. Through the Data API a profile row is readable by its owner and by anyone who added them. The card itself, name and score, is shown to anyone who has the name, see below. People are found only by exact username through `find_profile`, so there is no list of everyone to browse. Adding is one-way, like following. There is no global leaderboard: the score is worked out on the device, so the server cannot vouch for it, and between friends that does not matter. Ranking rules live in `src/lib/sync/board.ts`. A reader can set a display name and a card colour. Their card and score are at `/@username`, for anyone with the link. There is no private setting: following needs no approval, so it could not keep anyone out, and a switch would promise what it cannot do. The opt-in that matters is the global board, because that is where a name becomes findable. Groups are the shared kind of board: everyone in one sees everyone, the reader who made it is admin, and people join through an invite link that always expires (1, 7 or 30 days). An admin can also make a display link that shows the board read-only on a classroom screen without signing in. Group tables are closed to the Data API; every rule is in `functions/api/groups.ts`, and a trigger keeps an admin in every group that has members. Places on boards are worked out by Postgres views (`group_standings`, `global_standings`, in `drizzle/migrations/0015_standings_views.sql`) and read by the function only; `GET /boards` returns the reader's place on every board in one request. Plain views on purpose: either can become a materialized view or a trigger-kept table later without the function or the app changing. `/@username`, `/join/<code>` and `/display/<code>` are the pages that do not prerender: the static build serves them as `200.html` (rewritten in `vercel.json`).
...
- **Three inputs, one exercise.** A physical keyboard is read through `keydown`; a phone keyboard through `input` events on a focused field, because phone keyboards compose text and report most keydowns as `"Unidentified"`. A Japanese keyboard, a flick keypad or a romaji keyboard that composes kana, sends kana instead of letters: each kana is turned back into the keys that spell it (`src/lib/romaji/kana-keys.ts`) and fed through the same matcher, and deleting a kana takes back all its keys. The reader model keeps a baseline and a motor floor per input, `keyboard`, `touch` and `kana`, so grading and the score are fair on all three. The first key of a sentence decides which one it was. See `InputMethod` in `src/lib/srs/grade.ts`.
...
- **Corpus:** Sentences come from the Tatoeba Project under CC BY 2.0 FR, readings from kuromoji with IPADic, corrected against the Tatoeba Japanese indices where a human checked them. The attribution is a licence condition and stays visible on every page that shows sentences, which is every page but the classroom screen at `/display/<code>`.
...
- **Score:** one unbounded number the reader can compare with someone else, in `src/lib/stats/score.ts`. One number, no levels: a level needs boundaries and a score that climbs forever has none. It is the share of real text they can read: every item weighted by how often it turns up in the corpus (`static/corpus/text-share.json`, written by the corpus build) and by the chance they recall it, averaged over the next three days. That share goes on a scale of 1,000 points per halving of the words they would get stuck on, so it never stops growing and every point is harder than the last. It falls when they stop. It has no reading-speed term: a keystroke is hand plus eye, so speed across people cannot be measured fairly from typing. Chosen by simulation, not taste: see `scripts/simulate/README.md`, and test any change to it the same way. It is snapshotted into each attempt record as `readingScore`, because item state says what is true today and the past cannot be recomputed from it.
